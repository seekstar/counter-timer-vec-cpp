...
public:
	...
	const std::vector<counter_timer::Timer> &timers(size_t i) const {
		if (v_.size() <= i) {
			v_.lock();
			while (v_.size_locked() <= i) {
				v_.push_back_locked(
					new std::vector<counter_timer::Timer>(num_types_)
				);
			}
			v_.unlock();
		}
...
private:
	mutable rcu_vector_bp<std::vector<counter_timer::Timer> *> v_;
...
public:
	...
	const counter_timer::Timer &timer(size_t i, Type type) const {
		return v_.timers(i)[static_cast<size_t>(type)];
	}
private:
	TimersVector v_;
};
